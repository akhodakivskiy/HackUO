...
import kdkvsk.hackuo.network.packets.recvsend.{GiRequestPopupPacket, GiSelectPopupEntryPacket}
import kdkvsk.hackuo.network.packets.send.DoubleClickPacket
...
trait REPLHandlerOps {
  def handleCommand(w: World, command: List[String]): Response = {
    command match {
      ...
      case "paperdoll" :: Nil => PacketResponse(DoubleClickPacket(w.playerSerial.value))
      case "backpack" :: Nil => PacketResponse(DoubleClickPacket(w.player.backpackOpt.getOrElse(throw new IllegalStateException(s"player doesn't have a backpack")).serial.value))
