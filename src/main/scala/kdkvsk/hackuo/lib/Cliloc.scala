...
object Cliloc extends LazyLogging {
  def readIntBE(dis: DataInputStream): Int = {
    val b0: Byte = dis.readByte()
    val b1: Byte = dis.readByte()
    val b2: Byte = dis.readByte()
    val b3: Byte = dis.readByte()

    (((b0 << 0x18) | (b1 << 0x10)) | (b2 << 8)) | b3
  }

  def readIntLE(dis: DataInputStream): Int = {
    val b0: Byte = dis.readByte()
    val b1: Byte = dis.readByte()
    val b2: Byte = dis.readByte()
    val b3: Byte = dis.readByte()

    ((b0 | (b1 << 8)) | (b2 << 0x10)) | (b3 << 0x18)
  }
  ...
  def load(path: Path): Cliloc = {
    ...
    while (dis.available() > 0) {
      val number: Int = java.lang.Integer.reverseBytes(dis.readInt())
      val flag: Byte = dis.readByte()
      val length: Short = java.lang.Short.reverseBytes(dis.readShort())
