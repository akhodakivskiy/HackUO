...
case class ItemInfoPacket(serial: Serial, typeId: GraphicId, amount: Short, x: Short, y: Short, z: Byte, light: Byte, hue: Short, facing: Byte, flag: Byte) extends RecvPacket {
  def id: Int = ItemInfoPacketParser.packetId
...
object ItemInfoPacketParser extends RecvPacketParser {
  ...
  def parse(data: DataInputStream, size: Int): RecvPacket = {
    ...
    if (dataType == 0x0) {
      ItemInfoPacket(serial, graphicId, amount, x, y, z, light, hue, facing, flag)
    } else {
