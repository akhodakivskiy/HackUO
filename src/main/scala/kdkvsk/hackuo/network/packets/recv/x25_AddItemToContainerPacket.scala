...
case class x25_AddItemToContainerPacket(serial: Serial, graphicId: GraphicId, graphicIdOffset: Byte,
                                        amount: Short, x: Int, y: Int, containerSerial: Serial, hue: Short,
                                        gridIndexOpt: Option[Byte] = None) extends RecvPacket

object x25_AddItemToContainerPacketParser extends RecvPacketParser {
  ...
  def parse(data: DataInputStream, size: Int): RecvPacket = {
    ...
    x25_AddItemToContainerPacket(serial, graphicId, graphicIdOffset, amount, x, y, containerSerial, hue, slotIndexOpt)
  }
}
