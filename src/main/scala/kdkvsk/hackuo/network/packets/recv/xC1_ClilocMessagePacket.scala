...
case class xC1_ClilocMessagePacket(serial: Serial, bodyId: BodyId, location: Byte, hue: Short, font: Short, clilocId: Int, speakerName: String, args: String, textOpt: Option[String]) extends RecvPacket {
  def isSystem: Boolean = serial.value == 0xFFFFFFFF || bodyId.value == 0xFFFF
}

case class xC1_ClilocMessagePacketParser(cliloc: Cliloc) extends RecvPacketParser {
  ...
  def parse(data: DataInputStream, size: Int): RecvPacket = {
    ...
    xC1_ClilocMessagePacket(serial, bodyId, location, hue, font, clilocId, speakerName, args, textOpt)
  }
}
