...
case class CharacterListPacket(characters: Seq[CharacterName], locations: Seq[StartLocation], flags: Int, lastCharIdOpt: Option[Int]) extends RecvPacket {
  val id: Int = 0xA9
}

case class CharacterListPacketParser(newClient: Boolean) extends RecvPacketParser {
  ...
  def parse(data: DataInputStream, size: Int): RecvPacket = {
    ...
    CharacterListPacket(characters, locations, flags, None)
  }
}
