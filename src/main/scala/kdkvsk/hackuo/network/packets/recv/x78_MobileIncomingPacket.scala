...
case class x78_MobileIncomingPacket(serial: Serial, bodyId: BodyId, x: Short, y: Short, z: Short, facing: Byte,
                                    hue: Short, flags: Byte, notorietyByte: Byte,
                                    items: Seq[MobileIncomingItem]) extends RecvPacket {
  def id: Int = x78_MobileIncomingPacketParser.packetId
...
object x78_MobileIncomingPacketParser extends RecvPacketParser {
  ...
  def parse(data: DataInputStream, size: Int): RecvPacket = {
    ...
    x78_MobileIncomingPacket(serial, bodyId, x, y, z, facing, hue, statusFlag, notoriety, items.toVector)
  }
}
