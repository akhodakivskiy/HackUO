...
case class LoginDeniedPacket(reason: DenyReason.Type) extends RecvPacket

object LoginDeniedPacketParser extends RecvPacketParser {
  def packetId: Int = 0x82

  def parse(data: DataInputStream, size: Int): RecvPacket = {
    LoginDeniedPacket(DenyReason.apply(data.readByte()))
  }
}
