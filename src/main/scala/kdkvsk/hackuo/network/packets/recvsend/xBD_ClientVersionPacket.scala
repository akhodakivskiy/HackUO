...
case class xBD_ClientVersionPacket(version: String) extends SendPacket with RecvPacket {
  val id: Int = 0xBD
...
object xBD_ClientVersionPacketParser extends RecvPacketParser {
  ...
  def parse(data: DataInputStream, size: Int): RecvPacket = {
    ...
    if (length > 3) {
      ...
      xBD_ClientVersionPacket(version)
    } else {
      xBD_ClientVersionPacket("")
    }
  }
}
